...
{
	// getting command line input to generate port number
	if (argc != 2) {
	    fprintf(stderr,"Port name not specified\n");
	    exit(1);
	}

	char *endptr;
	long port_num = strtol(argv[1], &endptr, 10);
	uint16_t port;
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			if (!fp){
				char* error_message = "HTTP/1.1 404 File Not Found\r\n\r\n";
				send(new_fd,error_message,strlen(error_message),0);
				send(new_fd,"FILE NOT FOUND\n",15,0);
			}else{
